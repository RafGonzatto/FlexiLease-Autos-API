...
class ReserveService {
  ...
  async createReserve(
    ...
    const car = await this.carRepository.getCarById(
      reserveData.id_car.toString(),
    );
    if (!car) {
    ...
    const userReserves =
      await this.reserveRepository.getReservesByUserId(id_user);
    ...
    const carReservePeriod = await this.calculateDateDifference(
    ...
    const createdReserve =
      await this.reserveRepository.createReserve(reserveData);
  ...
  async getAllReserves(filters: any, limit?: number, offset?: number) {
    ...
    if (filters.id_user)
      whereConditions.id_user = new ObjectId(filters.id_user);
    if (filters.id_car) whereConditions._id = new ObjectId(filters.id_car);

    if (filters.id_user)
      whereConditions.id_user = {
        $regex: `.*${filters.id_user}.*`,
        $options: 'i',
      };
    if (filters.id_car)
      whereConditions.id_car = {
        $regex: `.*${filters.id_car}.*`,
        $options: 'i',
      };
    if (filters.start_date)
      whereConditions.start_date = {
        $regex: `.*${filters.start_date}.*`,
        $options: 'i',
      };
    if (filters.end_date)
      whereConditions.color = {
        $regex: `.*${filters.end_date}.*`,
        $options: 'i',
      };
    if (filters.final_value)
      whereConditions.final_value = {
        $regex: `.*${filters.final_value}.*`,
        $options: 'i',
      };
    ...
    const { reserves, total } =
      await this.reserveRepository.findAllWithPagination(options);
  ...
  async updateReserve(id: string, reserveData: any): Promise<IReserve | null> {
    const existingReserve: IReserve =
      await this.reserveRepository.getReserveById(id);
    if (!existingReserve) {
    ...
    let carReserves = await this.reserveRepository.getReservesByCarId(
      reserveData.id_car,
    );
    let userReserves = await this.reserveRepository.getReservesByUserId(
      reserveData.id_user,
    );

    carReserves = carReserves.filter(
      (reserve) => reserve._id !== new ObjectId(id),
    );
    userReserves = userReserves.filter(
      (reserve) => reserve._id !== new ObjectId(id),
    );
    ...
    const updatedReserve =
      await this.reserveRepository.updateReserve(existingReserve);
  ...
  private async checkReservationConflicts(
    ...
    for (const userReserve of userReserves) {
      ...
      if (
        ...
        throw new createError.BadRequest(
          'User already has a reservation for a car in the specified period',
        );
      }
    }
    ...
    for (const carReserve of carReserves) {
      ...
      if (
        ...
        throw new createError.BadRequest(
          'Car is already reserved in the specified period',
        );
      }
    }
  }
