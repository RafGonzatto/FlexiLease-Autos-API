...
class UserService {
  ...
  async createUser(userData: IUser): Promise<IUser> {
    ...
    const existingUser = await this.userRepository.findOneByConditions(emailConditions);
    if (existingUser) {
      throw new createError.Conflict('Email address is already registered');
    }
    const cpfConditions: any = { cpf: userData.cpf };
    const existingCPF = await this.userRepository.findOneByConditions(cpfConditions);
    if (existingCPF) {
    ...
    if (response.status !== 200) {
      throw new createError.NotFound('Failed to fetch address information from ViaCEP');
    }
  ...
  async updateUser(userData: IUser): Promise<IUser> {
    const existingUser  = await this.userRepository.getUserById(userData._id.toString());
    if (!existingUser ) {
    ...
    const existingEmail = await this.userRepository.findOneByConditions(emailConditions);
    if (existingEmail) {
      throw new createError.Conflict('Email address is already registered');
    }
  ...
  async getAllUsers(params: GetAllUsersParams): Promise<{ users: User[], total: number, limit: number, offset: number, offsets: number }> {
    ...
    if (birth) whereConditions.birth = { $regex: `.*${birth}.*`, $options: 'i' };
    if (email) whereConditions.email = { $regex: `.*${email}.*`, $options: 'i' };
    if (cep) whereConditions.cep = { $regex: `.*${cep}.*`, $options: 'i' };
    if (qualified) whereConditions.qualified = { $regex: `.*${qualified}.*`, $options: 'i' };
    if (patio) whereConditions.patio = { $regex: `.*${patio}.*`, $options: 'i' };
    if (complement) whereConditions.complement = { $regex: `.*${complement}.*`, $options: 'i' };
    if (neighborhood) whereConditions.neighborhood = { $regex: `.*${neighborhood}.*`, $options: 'i' };
    if (locality) whereConditions.locality = { $regex: `.*${locality}.*`, $options: 'i' };
    ...
    const { users, total } = await this.userRepository.findAllWithPagination(options);
