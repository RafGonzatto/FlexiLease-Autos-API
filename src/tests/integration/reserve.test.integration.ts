...
beforeAll(() => {
  ...
  createReserveService = new ReserveService(
    reservesRepository,
    carsRepository,
    usersRepository,
  );
...
describe('Reserve Service Integration Tests', () => {
  ...
  it('should be able to find all reserves with pagination', async () => {
    ...
    const filters = {};
    const { reserves, total } = await createReserveService.getAllReserves(
      filters,
      10,
      0,
    );
  ...
  it('should be able to get a reserve by ID', async () => {
    ...
    const foundReserve = await createReserveService.getReserveById(
      createdReserve._id.valueOf().toString(),
    );
