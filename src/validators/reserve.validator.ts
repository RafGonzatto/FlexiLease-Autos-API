...
const isValidDateFormat = (dateStr: string) => {
  ...
  return (
    isValid(parsedDate) &&
    parsedDate.getDate() === parseInt(dateStr.slice(0, 2), 10)
  );
};
...
const reserveSchema = z
  .object({
    ...
    start_date: z
      .string()
      .regex(/^\d{2}\/\d{2}\/\d{4}$/, {
        message: 'Start date must follow the format DD/MM/YYYY',
      })
      .nonempty({ message: 'Start date is required' })
      .refine(isValidDateFormat, {
        message: 'Invalid start date format. Please use the format DD/MM/YYYY',
      }),
    end_date: z
      .string()
      .regex(/^\d{2}\/\d{2}\/\d{4}$/, {
        message: 'End date must follow the format DD/MM/YYYY',
      })
      .nonempty({ message: 'End date is required' })
      .refine(isValidDateFormat, {
        message: 'Invalid end date format. Please use the format DD/MM/YYYY',
      }),
